...
const formatSalary = (salary) => {
  ...
  if (salary.min == null) return `Up to ${formatAmount(salary.max)} ${currency}`;
...
const formatPostedDate = (createdAt) => {
  ...
  if (elapsedMinutes < 60) return `Posted ${elapsedMinutes} minute${elapsedMinutes === 1 ? "" : "s"} ago`;

  const elapsedHours = Math.floor(elapsedMinutes / 60);
  if (elapsedHours < 24) return `Posted ${elapsedHours} hour${elapsedHours === 1 ? "" : "s"} ago`;

  const elapsedDays = Math.floor(elapsedHours / 24);
  if (elapsedDays < 7) return `Posted ${elapsedDays} day${elapsedDays === 1 ? "" : "s"} ago`;
...
const JobCard = ({ job }) => {
  ...
  return (
    <article className="listing-job-card">
      ...
      <div className="company-mark listing-company-mark">{companyLogo ? <img src={companyLogo} alt={`${companyName} logo`} /> : companyMark}</div>
      <p className="company-name">{companyName}<span className="verified">&#10003;</span></p>
      <h2>{job.title}</h2>
      <div className="listing-meta"><span><PinIcon />{job.location || "Location not specified"}</span><span><strong>◆</strong>{job.employmentType || "Employment type not specified"}</span><span><strong>◈</strong>{formatSalary(job.salary)}</span></div>
      <div className="skills">{skills.map((skill, index) => <span key={`${skill}-${index}`}>{skill}</span>)}</div>
    </Link>
    <div className="listing-job-footer"><span>{formatPostedDate(job.createdAt)}</span><Link to={`/jobs/${jobId}`} className="text-link">View Job <ArrowIcon /></Link></div>
    </article>
  );
};
